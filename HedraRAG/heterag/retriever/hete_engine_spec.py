...
class DenseHeteEngineSpec(BaseEngine):
    ...
    def reinit(self, config):
        ...
        self.spec_total_size = self.config["spec_total_size"]
        ...
        if "IVF" in self.indextype:
            ...
            self.num_sub_stages = self.total_nprobe / config["nprobe_minibatch"]
        ...
        self.spec_method = config["spec_method"] if config["spec_method"] is not None else "heteRAG"
        self.wrong_spec = 0
    ...
    def step(self):
        ...
        if "IVF" in self.indextype:
                ...
                for request, assigned_centroid, centroid_distance, query_emb, cluster_size in zip(new_list, batch_assigned_centroids, centroid_distances, new_batch_emb, cluster_size_list):
                    ...
                    if qid in self.history_info_dict:
                        ...
                        sorted_indices = sorted(
                            indices,
                            key=lambda i: (0 if assigned_centroid[i] in hit_set else 1))
            ...
            if len(query_emb):
                ...
                topk_score, topk_id, cmin_dist, cmin_id = \
                    self.index.search_preassigned_heterag(query_emb, self.large_topk, cluster_to_process, cluster_dist_to_process, D = topk_score, I = topk_id, init_heap = False)
                ...
                for qid, qtopkid_large, qtopkscore_large, qcmin_dist in zip(qid_list, topk_id, topk_score, cmin_dist):
                    ...
                    if this_request.is_empty:
                        ...
                        if this_request.spec_begin and new_taskid.need_regeneration:
                            self.wrong_spec += 1
                ...
                # heteRAG: speculative retrieval, sending $spec_total_size$ retrieval results for each step
                
                if self.spec_method == "heteRAG":

                    while self.spec_total_size > 0 and len(idx_list) < self.spec_total_size:
                        min_topk = sys.float_info.max
                        min_qid = -1

                        for qid in qid_list:
                            if qid in self.request_dict:
                                this_request = self.request_dict[qid]
                ...
                # RAGCache: speculative retrieval, sending $spec_total_size$ retrieval results for each step

                elif self.spec_method == "RAGCache":
                    for qid in qid_list:
                        if qid in self.request_dict:
                            this_request = self.request_dict[qid]
                            if self.request_dict[qid].subset_iter == 4:
                                self.request_dict[qid].spec_exec()
                                idx_list.append(self.request_dict[qid].spec_idx)
                                new_taskid = self.request_dict[qid].taskid
                                new_taskid.is_spec = True
                                new_taskid.begin_spec = True
                                taskid_list.append(new_taskid)
                    ...
                    raise ValueError("Wrong spec method!")
