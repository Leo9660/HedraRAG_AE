...
def main():
    from heterag import Config, HeteragExecutor, SequentialRAGraph, get_dataset, SequentialExecutor, Request
    import numpy as np
    import faiss
    import argparse
    import time
    ...
    for rps in rps_list:
        ...
        if args.write_file is not None:
            with open(args.write_file, 'a+', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["FlashRAG", args.rag_workflow1, args.rag_workflow2, large_small_workflow, small_workflow, args.request_per_second, args.nprobe, average_query_latency, loop_time])
