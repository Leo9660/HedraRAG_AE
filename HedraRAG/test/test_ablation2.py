def main():
    ...
    parser.add_argument("--continuous_retrieval", type=bool, default=True)
    ...
    parser.add_argument("--spec_output_file", type=str, default=None)
    ...
    config_dict = {
        ...
        "iter_num": 2,
        "print_log": True,
        "continuous_retrieval": args.continuous_retrieval,
    ...
    test_data = all_split["test"][0:256]
    ...
    rps = args.request_per_second

    Request.request_num = 0
    print(f"workflow={args.rag_workflow}, data_dir={args.data_dir}, rps={rps}")
    ...
    for key, values in executor.request_time_dict.items():
        current_latency = values - start_time_list[key] - t1
        query_latency.append(current_latency)
    average_query_latency = np.mean(query_latency)
    average_s_per_token = np.mean(s_per_token)
    ...
    if args.write_file is not None:
        with open(args.write_file, 'a+', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([args.spec_method, args.spec_total_size, rps, average_query_latency])
