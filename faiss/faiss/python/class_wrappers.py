...
def handle_Index(the_class):
    ...
    #heterag
    def replacement_search_with_cluster_id(self, x, k, *, D=None, I=None, init_heap = True, params=None):
        n, d = x.shape
        x = np.ascontiguousarray(x, dtype='float32')
        assert d == self.d
        nprobe = self.nprobe
        ...
        DCLUSTER = np.empty((n, nprobe), dtype=np.float32)
        ICLUSTER = np.empty((n, nprobe), dtype=np.int64)

        self.search_with_cluster_id_c(n, swig_ptr(x), k, swig_ptr(D), swig_ptr(I), swig_ptr(DCLUSTER), swig_ptr(ICLUSTER), init_heap, params)
        return D, I, DCLUSTER, ICLUSTER

    #heterag
    def replacement_search_with_idx(self, x, start, end, k, *, params=None, D=None, I=None):
        n, d = x.shape
        x = np.ascontiguousarray(x, dtype='float32')
        assert d == self.d
        assert k > 0
        ...
        if I is None:
            ...
            assert I.shape == (n, k)
        self.search_with_idx_c(n, swig_ptr(x), start, end, k, swig_ptr(D), swig_ptr(I), params)
        return D, I
    ...
    #heterag
    def replacement_search_preassigned_heterag(self, x, k, Iq, Dq, *, params=None, D=None, I=None, init_heap=True):
        ...
        lengths = np.array([len(arr) for arr in Iq], dtype='int64')
        query_nprobe = np.concatenate([[0], np.cumsum(lengths)])

        flat_list = [item for sublist in Iq for item in sublist]
        Iq = np.array(flat_list, dtype='int64')
        flat_list = [item for sublist in Dq for item in sublist]
        Dq = np.array(flat_list, dtype='float32')
        ...
        # DCLUSTER = np.empty((n, nprobe), dtype=np.float32)
        # ICLUSTER = np.empty((n, nprobe), dtype=np.int64)
        DCLUSTER = np.empty_like(Iq, dtype=np.float32)
        ICLUSTER = np.empty_like(Dq, dtype=np.int64)
        ...
        #     n, swig_ptr(x),
        #     k,
        #     swig_ptr(Iq), swig_ptr(Dq), swig_ptr(query_nprobe),
        #     swig_ptr(D), swig_ptr(I),
        #     swig_ptr(DCLUSTER), swig_ptr(ICLUSTER),
        #     init_heap,
        #     False
        # )
        # print("Actual input", Iq, Dq, query_nprobe)

        self.search_preassigned_heterag_omp(
            n, swig_ptr(x),
            k,
            swig_ptr(Iq), swig_ptr(Dq), swig_ptr(query_nprobe),
            swig_ptr(D), swig_ptr(I),
        ...
        DCLUSTER_ret = split_by_lengths(DCLUSTER, lengths)
        ICLUSTER_ret = split_by_lengths(ICLUSTER, lengths)

        return D, I, DCLUSTER_ret, ICLUSTER_ret

    def replacement_get_cluster_size_heterag(self, Iq):
        lengths = np.array([len(arr) for arr in Iq], dtype='int64')
        n = np.sum(lengths)
        # print(n)
        # query_nprobe = np.concatenate([[0], np.cumsum(lengths)])
        ...
        flat_list = [item for sublist in Iq for item in sublist]
        Iq_flat = np.array(flat_list, dtype='int64')

        flat_list = [item for sublist in Iq for item in sublist]
        cluster_size = np.empty_like(Iq_flat, dtype=np.int64)

        self.get_cluster_size_heterag2(int(n), swig_ptr(Iq_flat), swig_ptr(cluster_size))

        offsets = np.cumsum(lengths)

        cluster_size = np.split(cluster_size, offsets[:-1])

        return cluster_size
    
    def replacement_get_cluster_ptr_heterag(self, Iq):
        Iq = np.array(Iq, dtype='int64')
        n = Iq.shape[0]
        # cluster_ptr = np.array_like(Iq, dtype='int64')
        cluster_ptr = np.empty_like(Iq, dtype=np.int64)

        self.get_cluster_ptr_heterag_c(n, swig_ptr(Iq), swig_ptr(cluster_ptr))
    ...
    #heterag
    replace_method(the_class, 'search_with_idx', replacement_search_with_idx, ignore_missing=True)
    replace_method(the_class, 'search_with_cluster_id', replacement_search_with_cluster_id, ignore_missing=True)
    ...
    #heterag
    replace_method(the_class, 'search_preassigned_heterag',
                   replacement_search_preassigned_heterag, ignore_missing=True)
    replace_method(the_class, 'get_cluster_size_heterag',
                   replacement_get_cluster_size_heterag, ignore_missing=True)
    replace_method(the_class, 'get_cluster_ptr_heterag',
                   replacement_get_cluster_ptr_heterag, ignore_missing=True)
