...
// heterag
void IndexIVF::search_preassigned_heterag_omp(
    idx_t n,
    const float* x,
    idx_t k,
    const idx_t* assign,
    const float* centroid_dis,
    const idx_t* query_nprobe,
    float* distances,
    idx_t* labels,
    float* cluster_min,
    idx_t* cid_min,
...
auto sub_search_func = [this, k, nprobe, params](
    ...
    double t1 = getmillisecs();
    search_preassigned_heterag_nprobe(
            n,
            x,
            k,
            assign,
...
#pragma omp parallel for if (nt > 1)
    for (idx_t slice = 0; slice < nt; slice++) {
        ...
        if (i1 > i0) {
            try {
                ...
                //         x + i0 * d,
                //         assign + i0 * nprobe,
                //         centroid_dis + i0 * nprobe,
                //         distances + i0 * k,
                //         labels + i0 * k,
                //         cluster_min + i0 * nprobe,
                //         cid_min + i0 * nprobe,
                //         init_heap,
                //         &stats[slice]);
                sub_search_func(
                        i1 - i0,
                        x + i0 * d,
                        assign + query_nprobe[i0],
                        centroid_dis + query_nprobe[i0],
                        query_nprobe + i0,
                        distances + i0 * k,
                        labels + i0 * k,
                        cluster_min + query_nprobe[i0],
                        cid_min + query_nprobe[i0],
                        init_heap,
                        &stats[slice]);
            } catch (const std::exception& e) {
                std::lock_guard<std::mutex> lock(exception_mutex);
    ...
    // handle parallelization at level below (or don't run in parallel at
    // all)
    sub_search_func(n, x, assign, centroid_dis, query_nprobe, distances, labels, cluster_min, cid_min, init_heap, &indexIVF_stats);
}
}

//heterag
void IndexIVF::search_with_cluster_id(
    idx_t n,
    const float* x,
    idx_t k,
    float* distances,
    ...
    IndexIVFStats* stats) const {
        ...
        #pragma omp parallel for if (nt > 1)
            ...
            // handle parallelization at level below (or don't run in parallel at
            // all)
            sub_search_func(n, x, distances, labels, cluster_min, cid_min, init_heap, &indexIVF_stats);
        }
}
...
//heterag
void IndexIVF::search_preassigned_heterag(
    idx_t n,
    const float* x,
    idx_t k,
    const idx_t* keys,
    ...
    auto scan_one_list = [&](idx_t key,
        ...
        try {
            if (invlists->use_iterator) {
                ...
                // printf("cluster %d %.5f\n", list_size, codes[0]);
                nheap += scanner->scan_codes_heterag(
                        list_size, codes, ids, simi, idxi, cluster_min, cid_min, k);
...
//heterag
void IndexIVF::get_cluster_size_heterag(
    idx_t n,
    idx_t* keys,
    idx_t* cluster_size
) const {
    if (invlists->use_iterator) {
        ...
        for (idx_t i = 0; i < n; i++)
        {
            size_t list_size = invlists->list_size(keys[i]);
        }
    }
...
//heterag
void IndexIVF::search_preassigned_heterag_nprobe(
    idx_t n,
    const float* x,
    idx_t k,
    const idx_t* keys,
...
#pragma omp for
        for (idx_t i = 0; i < n; i++) {
            ...
            idx_t this_query_nprobe = query_nprobe[i + 1] - query_nprobe[i];
            idx_t this_query_nprobe_start = query_nprobe[i] - query_nprobe[0];

            // printf("this query nprobe %d %d\n", this_query_nprobe, this_query_nprobe_start);

            // loop over probes
            for (size_t ik = 0; ik < this_query_nprobe; ik++) {
                size_t list_size = invlists->list_size(keys[this_query_nprobe_start + ik]);
                // printf("probe size %d\n", list_size);
                // printf("loop over %d\n", this_query_nprobe_start + ik);

                nscan += scan_one_list(
                    keys[this_query_nprobe_start + ik],
                    coarse_dis[this_query_nprobe_start + ik],
                    simi,
                    idxi,
                    cluster_min + this_query_nprobe_start + ik,
                    cid_min + this_query_nprobe_start + ik,
                    max_codes - nscan);
                // nscan += scan_one_list(
                //         keys[this_query_nprobe_start + ik],
                //         coarse_dis[this_query_nprobe_start + ik],
...
size_t InvertedListScanner::scan_codes_heterag(
    size_t list_size,
    const uint8_t* codes,
    const idx_t* ids,
    float* simi,
