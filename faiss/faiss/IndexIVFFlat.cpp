...
//heterag
void IndexIVFFlat::get_cluster_size_heterag2(
    int n,
    idx_t* keys,
    idx_t* cluster_size
) const {
...
void IndexIVFFlat::get_cluster_ptr_heterag(
    int n,
    idx_t* cluster_idx,
    idx_t* cluster_ptr
) const {
    for (int i = 0; i < n; i++)
    {
        idx_t key = cluster_idx[i];
        InvertedLists::ScopedCodes scodes(invlists, key);
        const uint8_t* codes = scodes.get();

        cluster_ptr[i] = (idx_t)codes;
    }
}
