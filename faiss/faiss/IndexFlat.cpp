...
void IndexFlat::search_with_idx(
    idx_t n,
    const float* x,
    idx_t start,
    idx_t end,
    ...
    const SearchParameters* params) const
{
    ...
    idx_t search_total = end - start;

    // we see the distances and labels as heaps
    if (metric_type == METRIC_INNER_PRODUCT) {
        float_minheap_array_t res = {size_t(n), size_t(k), labels, distances};
        knn_inner_product(x, get_xb() + start * d, d, n, search_total, &res, sel);
    } else if (metric_type == METRIC_L2) {
        float_maxheap_array_t res = {size_t(n), size_t(k), labels, distances};
        knn_L2sqr(x, get_xb() + start * d, d, n, search_total, &res, nullptr, sel);
    } else {
        FAISS_THROW_IF_NOT(!sel); // TODO implement with selector
        knn_extra_metrics(
                x,
