...
int faiss_IndexFlat_search_with_idx(
    const FaissIndexFlat* index,
    idx_t n,
    const float* x,
    idx_t start,
    idx_t end,
    ...
    idx_t* labels) {
try {
    reinterpret_cast<const faiss::IndexFlat*>(index)->search_with_idx(
            n, x, start, end, k, distances, labels);
}
CATCH_AND_HANDLE
}
