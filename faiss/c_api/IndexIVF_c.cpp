...
int faiss_IndexIVF_search_with_cluster_id(
    const FaissIndexIVF* index,
    idx_t n,
    const float* x,
    idx_t k,
...
{
    try {
        reinterpret_cast<const IndexIVF*>(index)->search_with_cluster_id(
                n, x, k, distances, labels, cluster_min, cid_min, init_heap);
    }
    CATCH_AND_HANDLE
}

// int faiss_IndexIVF_search_preassigned_heterag_api(
//     const FaissIndexIVF* index,
//     idx_t n,
//     const float* x,
//     idx_t k,
...
//     try {
//         printf("omp");
//         reinterpret_cast<const IndexIVF*>(index)->search_preassigned_heterag_omp(
//                 n, x, k, assign, centroid_dis, distances, labels, cluster_min, cid_min, init_heap, store_pairs);
//     }
//     CATCH_AND_HANDLE
// }

int faiss_IndexIVF_get_cluster_size(
    const FaissIndexIVF* index,
    idx_t n,
    idx_t* keys,
    idx_t* cluster_size
...
{
    try {
        reinterpret_cast<const IndexIVF*>(index)->get_cluster_size_heterag(
                n, keys, cluster_size);
    }
    CATCH_AND_HANDLE
}
